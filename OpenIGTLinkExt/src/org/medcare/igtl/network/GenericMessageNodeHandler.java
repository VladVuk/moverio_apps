...
public class GenericMessageNodeHandler {
	...
	public OpenIGTMessage perform(String messageType,Header head, byte[] body, IOpenIgtPacketListener node) throws Exception {
        ...
        // Added by Long Qian, to accommodate PolyData messages
        else if (messageType.equals("POLYDATA")) {
        	openIGTMessage = new PolyDataMessage(head, body);
        	openIGTMessage.Unpack();
        	PolyData pd = ((PolyDataMessage)openIGTMessage).getPolyData();
        	System.out.println("Received PolyData message");
        	node.onRxPolyData(openIGTMessage.getDeviceName(), pd);
        }
        else {
