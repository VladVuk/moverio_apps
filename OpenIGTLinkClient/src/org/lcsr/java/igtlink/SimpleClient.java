...
public class SimpleClient implements IOpenIgtPacketListener {
	/**
	 * @param args
	 */
	...
	public static void main(String[] args) {
		...
		SimpleClient.parseXMLStringMessage(msg);
		...
		try {
			...
			for(int i=0;i<500;i++){
				...
				position[0] = count;
				position[1] = count;
				position[2] = (int)(Math.random()*100);
				double rotation[][] = t.getRotationMatrixArray();
				rotation[0][1] = Math.random();
				count = count + 1;
				client.sendMessage(new TransformMessage("PC_" + String.valueOf(count), position ,rotation ));
				Thread.sleep(1000);
			}
			client.stopClient();
